#include "admin_tech_med.h"
#include "ui_admin_tech_med.h"
#include "admin_homepage.h"
#include "ui_admin_homepage.h"

#include <QSqlDatabase>

admin_tech_med::admin_tech_med(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::admin_tech_med)
{
    ui->setupUi(this);
...
admin_tech_med::~admin_tech_med()
{
    delete ui;
}

void admin_tech_med::on_submitbtn_clicked()
{
    {
    ...
    //        qDebug()<<"DB is connceted is failed";
        ...
        QSqlQuery qry_qn;
         qry_qn.prepare("insert into question_tech_med(qn,qn_no,choice_1,choice_2,choice_3,choice_4,crt_choice)values ('"+qn_editor+"','"+qn_number+"','"+choice_1+"','"+choice_2+"','"+choice_3+"','"+choice_4+"','"+crt_choice+"')");
        if(qry_qn.exec())
        {
...
void admin_tech_med::on_quit_clicked()
{
    QApplication::quit();
}
