...
void Adminedit_verbal::on_submitbtn_clicked()
{
    {
    ...
    //        qDebug()<<"DB is connceted is failed";
        ...
        QSqlQuery qry_qn;
        qry_qn.prepare("insert into question_verbal_easy(qn,qn_no,choice_1,choice_2,choice_3,choice_4,crt_choice)values ('"+qn_editor+"','"+qn_number+"','"+choice_1+"','"+choice_2+"','"+choice_3+"','"+choice_4+"','"+crt_choice+"')");
        if(qry_qn.exec())
        {
...
void Adminedit_verbal::on_quit_clicked()
{
    QApplication::quit();
}
