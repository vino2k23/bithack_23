#include "admin_verbal_hard.h"
#include "ui_admin_verbal_hard.h"
...
admin_verbal_hard::admin_verbal_hard(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::admin_verbal_hard)
{
    ui->setupUi(this);
...
admin_verbal_hard::~admin_verbal_hard()
{
    delete ui;
}

void admin_verbal_hard::on_submitbtn_clicked()
{
    {
    ...
    //        qDebug()<<"DB is connceted is failed";
        ...
        QSqlQuery qry_qn;
        qry_qn.prepare("insert into question_verbal_hard(qn,qn_no,choice_1,choice_2,choice_3,choice_4,crt_choice)values ('"+qn_editor+"','"+qn_number+"','"+choice_1+"','"+choice_2+"','"+choice_3+"','"+choice_4+"','"+crt_choice+"')");
        if(qry_qn.exec())
        {
...
void admin_verbal_hard::on_quit_clicked()
{
    QApplication::quit();
}
